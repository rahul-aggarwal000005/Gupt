...
export const registerPasskey = async (): Promise<boolean> => {
  try {
    ...
    const optionsResp = await api.get('/api/auth/webauthn/register/generate-options');
    ...
    const verificationResp = await api.post('/api/auth/webauthn/register/verify', attResp);
...
export const loginWithPasskey = async (email: string): Promise<any> => {
  try {
    ...
    const optionsResp = await api.get(`/api/auth/webauthn/login/generate-options?email=${encodeURIComponent(email)}`);
    const options = optionsResp.data;
